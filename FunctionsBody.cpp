...
string UncomplitedList::AddNoteToUncomplited(string newnote) {
	...
	SYSTEMTIME st = { 0 };
	GetLocalTime(&st);
	string timeOfUncomplited = to_string(st.wHour) + ':' + to_string(st.wMinute) + ':' + to_string(st.wSecond) + "  " + to_string(st.wDay) + '.' +to_string(st.wDay) + '.'+to_string(st.wYear);
	...
	if (WriteInUncomplitedFile.is_open()) {
		WriteInUncomplitedFile << newnote <<'(' << timeOfUncomplited << ')' << endl;
	}
...
void ComplitedList::AddNoteToComplited(int note_to_add, vector <string> savedUncomplitedNotes) {
	...
	SYSTEMTIME st = { 0 };
	GetLocalTime(&st);
	string timeOfComplited = to_string(st.wHour);
	timeOfComplited += ':';
	timeOfComplited += to_string(st.wMinute);
	timeOfComplited += ':';
	timeOfComplited += to_string(st.wSecond);
	...
	if (WriteInComplitedFile.is_open()) {
		WriteInComplitedFile << savedUncomplitedNotes[note_to_add - 1] << '(' << timeOfComplited << ')' << endl;
	}
...
void ToDoList::Print(int action) {
	...
	if (action == 1) {
		...
		getline(cin, your_note);
		if(your_note.size() != 0) cout << uncomplitedMember.AddNoteToUncomplited(your_note) << endl;
	}
	else if (action == 2) {
		string note_to_find;
...
void UI::SetCursorVisibility(bool show, short size) {
...
void UI::SetActiveTextColor() {
	SetConsoleTextAttribute(hStdOut, FOREGROUND_BLUE | FOREGROUND_INTENSITY);
}

void UI::SetInactiveTextColor() {
